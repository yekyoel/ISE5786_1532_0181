...
public class Cylinder extends Tube {
    ...
    public geometries.api.BoundingBox getBoundingBox() {
        Point p0 = _axis.origin();
        Point p1 = p0.add(_axis.direction().scale(_height));
        double minX = Math.min(p0.getX(), p1.getX()) - _radius;
        double minY = Math.min(p0.getY(), p1.getY()) - _radius;
        double minZ = Math.min(p0.getZ(), p1.getZ()) - _radius;
        double maxX = Math.max(p0.getX(), p1.getX()) + _radius;
        double maxY = Math.max(p0.getY(), p1.getY()) + _radius;
        double maxZ = Math.max(p0.getZ(), p1.getZ()) + _radius;
        return new geometries.api.BoundingBox(
                new primitives.Point(minX, minY, minZ),
                new primitives.Point(maxX, maxY, maxZ));
    }
}
